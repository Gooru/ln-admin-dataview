...
export default Ember.Component.extend(ModalMixin, {
   ...
   */
  isFilterSelected: function(textToValidate, filterItems) {
  ...
  fetchCategoryFilters: function() {
    ...
    taxonomyCategories.map(category =>  {
      ...
      if (selectedFilterItems.category !== undefined && selectedFilterItems.category.length > 0) {
      ...
      return Ember.RSVP.hash({
        ...
        .then(function(hash) {
          hash.subjectList.map(subject=> {
            ...
            if (selectedFilterItems.subject !== undefined && selectedFilterItems.subject.length > 0) {
      ...
      return Ember.RSVP.hash({
        ...
        .then(function(hash) {
          hash.courseList.map(course => {
            ...
            if (selectedFilterItems.course !== undefined && selectedFilterItems.course.length > 0) {
              courseInfo.checked = component.isFilterSelected(course.code, selectedFilterItems.course);
            }
    ...
    return Ember.RSVP.hash({
      ...
      .then(function(hash) {
        hash.audienceFilters.map(audience => {
          ...
          if (selectedFilterItems.audience !== undefined && selectedFilterItems.audience.length > 0) {
            audienceInfo.checked = component.isFilterSelected(audience.id, selectedFilterItems.audience);
          }
   ...
   */
  fetchCenturySkillFilters: function() {
    let component = this;
    let filterList = [];
    let userId = component.get('session.id');
    ...
    if (component.get('isToggleExpanded')) {
      ...
      return Ember.RSVP.hash({
        ...
        .then(function(hash) {
          ...
          component.send('showModal', 'modals.activity.century-skills', modalData);
        });
    } else {
      ...
      .then(function(hash) {
        hash.licenseList.map(license => {
          ...
          if (selectedFilterItems.licenses !== undefined && selectedFilterItems.licenses.length > 0) {
            licenseInfo.checked = component.isFilterSelected(license.id, selectedFilterItems.licenses);
          }
    ...
    return Ember.RSVP.hash({
      ...
      .then(function(hash) {
        hash.dokItems.map(dok => {
          ...
          if (selectedFilterItems.dok !== undefined && selectedFilterItems.dok.length > 0) {
            dokInfo.checked = component.isFilterSelected(dok.id, selectedFilterItems.dok);
          }
   ...
   */
  fetchPublisherFilters: function() {
    let component = this;
    let filterList = [];
    let userId = component.get('session.id');
    let selectedFilterItems = JSON.parse(localStorage.getItem(`research_${userId}_activities_filters`)) || component.get('selectedFilterItems');
    ...
    return Ember.RSVP.hash({
      ...
      .then(function(hash) {
        hash.publisherList.map(publisher => {
          ...
          if (selectedFilterItems.publisher !== undefined && selectedFilterItems.publisher.length > 0) {
            publisherInfo.checked = component.isFilterSelected(publisher.providerId, selectedFilterItems.publisher);
          }

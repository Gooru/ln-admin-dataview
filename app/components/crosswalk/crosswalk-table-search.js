...
export default Ember.Component.extend({
  ...
  getMatchingCrosswalkCodes(crosswalkCode, searchInput) {
    ...
    return searchPattern.test(crosswalkCode.id) || searchPattern.test(crosswalkCode.title);
  },
  ...
  fetchLearningMapContentByStandardId(crosswalkCode) {
    ...
    let learningMapContentPromise = Ember.RSVP.resolve(component.get('searchService').learningMapsContent(crosswalkCode, length));
  ...
  getContentCountData(contentCount) {
    ...
    let courseCount = contentCount.course ? contentCount.course.totalHitCount : 0;
    let unitCount = contentCount.unit ? contentCount.unit.totalHitCount : 0;
    let lessonCount = contentCount.lesson ? contentCount.lesson.totalHitCount : 0;
    let collectionCount = contentCount.collection ? contentCount.collection.totalHitCount : 0;
    let assessmentCount = contentCount.assessment ? contentCount.assessment.totalHitCount : 0;
    let resourceCount = contentCount.resource ?contentCount.resource.totalHitCount : 0;
    let questionCount = contentCount.question ? contentCount.question.totalHitCount : 0;
    let rubricCount = contentCount.rubric ? contentCount.rubric.totalHitCount : 0;

...
export default Ember.Component.extend({
  ...
  getSearchContentCount: function() {
    ...
    const culacCountPromise = component
      .get('activityService')
      .getLearningMaps(appliedFilters, term);
    return Ember.RSVP.hash({
      ...
      let collectionCounts = hash.culacCount
        .get('collection')
        .get('totalHitCount');
      let assessmentCounts = hash.culacCount
        .get('assessment')
        .get('totalHitCount');
      ...
      filterTypes.map(filterTypeInfo => {
        ...
        if (categorizedFilter) {
          formattedFilters = Object.assign(
            formattedFilters,
            component.getFormattedSearchFilters(filterType, categorizedFilter)
          );
        }
      });
    }

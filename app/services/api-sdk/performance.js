...
import PerformanceAdapter from 'admin-dataview/adapters/performance/performance';
import PerformanceSerializer from 'admin-dataview/serializers/performance/performance';
/**
...
export default Ember.Service.extend({
  ...
  init: function() {
    ...
    this.set('performanceAdapter', PerformanceAdapter.create(Ember.getOwner(this).ownerInjection()));
    this.set('performanceSerializer', PerformanceSerializer.create(Ember.getOwner(this).ownerInjection()));
  },
  ...
  getUserPerformanceCollections: function(userId, courseId, unitId, lessonId,  classId) {
    ...
    return new Ember.RSVP.Promise(function(resolve, reject) {
      service
        .get('performanceAdapter')
        .getUserPerformanceCollections(userId, courseId, unitId, lessonId, classId)
        .then(function(response) {
          resolve(service.get('performanceSerializer').normalizeUserPerformanceCollections(response));
        }, reject);
    });
  },
